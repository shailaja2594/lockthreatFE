...
var Scheduler = Widget.inherit({
    ...
    _init: function() {
        ...
        this._customizeStoreLoadOptions();
        this.$element().addClass(WIDGET_CLASS).toggleClass(WIDGET_WIN_NO_TOUCH_CLASS, !!(browser.msie && support.touch));
        this._initEditing();
    ...
    _processActionResult: function(actionOptions, callback) {
        ...
        if (typeUtils.isPromise(actionOptions.cancel)) {
            when(deferredUtils.fromPromise(actionOptions.cancel)).always(function(cancel) {
                if (!typeUtils.isDefined(cancel)) {
                    cancel = "rejected" === actionOptions.cancel.state()
                }
                callback.call(_this7, cancel)
            })
        } else {
    ...
    setTargetedAppointmentResources: function(targetedAppointment, appointmentElement, appointmentIndex) {
        ...
        if (groups && groups.length) {
            ...
            if (this._isAgenda()) {
                ...
                getGroups = function() {
                    var setting = $(appointmentElement).data("dxAppointmentSettings") || {};
